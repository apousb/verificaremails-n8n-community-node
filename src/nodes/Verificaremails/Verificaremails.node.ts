...
export class Verificaremails implements INodeType {
	description: INodeTypeDescription = {
		...
		group: ['transform'],
		version: 1,
		description: 'Validate emails, phones, names and addresses with VerificarEmails.com API',
		defaults: {
			name: 'VerificarEmails',
		},
		inputs: ['main'],
		...
		properties: [
			...
			// Advanced name services
			{
				...
				displayOptions: {
					show: {
						service: ['name_correction', 'name_autocomplete'],
					},
				},
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const apiKey = (credentials as any).apiKey as string;
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				switch (service) {
					...
					case 'name_correction': {
						...
						const payload: Record<string, unknown> = {
							name: nameInput,
							use_first_names: Number(useFirstNames),
						};
				...
				// The helper should internally use this.helpers.httpRequest()
				// and include Authorization with the apiKey.
				// Signature expected: verificaremailsApiRequest.call(this, method, term, apiKey, service)
				// Adjust if your GenericFunctions differs.
				// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
				const response: any = await (verificaremailsApiRequest as any).call(this, 'GET', term, apiKey, service);

				const status =
					response?.result?.status ?? response?.status ?? 'unknown';
